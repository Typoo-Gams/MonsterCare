...
public class InfoPanel_Button : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        string text = string.Format("HomePrototype\nHealth: {0}\nEnergy: {1}\n Hunger: {2}\n Sleep: {3}\nLast save at: \n{4}\nTime in seconds since last played: {5}\nHappiness: {6}",
                    Math.Truncate(manager.ActiveMonster.HealthStatus), Math.Truncate(manager.ActiveMonster.EnergyStatus) ,  Math.Truncate(manager.ActiveMonster.HungerStatus), Math.Truncate(manager.ActiveMonster.SleepStatus), lastPlay, Math.Truncate(lastPlayInSec), Math.Truncate(manager.ActiveMonster.HappinessStatus));
        Stats.text = text;
    }
